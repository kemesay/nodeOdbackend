...
const fromEmail = "bookingnotification@ethiosmartride.com";
// const adminEmail = "abditirunehdev@gmail.com";
const adminEmail = "info@ethiosmartride.com";
...
// SMTP configuration
const smtpConfig = {
  host: "mail.ethiosmartride.com",
  port: 465,
  secure: true, // true for 465, false for other ports
  auth: {
    user: "bookingnotification@ethiosmartride.com",
    pass: "@Mesay123",
...
async function sendBookingApprovalEmail(data, action, userEmail) {
  ...
  const options = {
    from: fromEmail,
    to: userEmail,
    subject: "Ethio-Smart-Ride Notifications",
    html: html,
  };
...
async function sendResetPasswordEmail(email, fullName, resetToken) {
  // Custom URL for deep linking to Flutter app
  // const resetLinkFlutterApp = `myapp://reset-password?token=${resetToken}`;
  
  // Fallback web URL (in case the app is not installed)
  const resetLink = `https://api.ethiosamrtride.com/reset-password/${resetToken}`;

  // Variables for email template
  const variables = {
    fullName: fullName,
    // resetLinkFlutterApp: resetLinkFlutterApp,
    resetLink: resetLink,
  };
