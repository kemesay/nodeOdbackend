...
const AirportBook = sequelize.define(
  "AirportBook",
  {
    ...
    /** When airportId is null: terminal / airport side from maps (same pattern as accommodation) */
    ...
    // Admin manual discount only (see applyDiscountToAirportBook). Kept
...
const validateAirportBook = Joi.object({
  ...
  carId: Joi.number().integer().allow(null),
  gratuityId: Joi.number().integer().allow(null),
  airportId: Joi.number().integer().positive().allow(null),
  /** Use with lat/lng when not sending airportId (free-form airport / terminal from Places API) */
  airportLocationAddress: Joi.string().allow("", null),
  airportLocationLatitude: Joi.number().allow(null),
  airportLocationLongitude: Joi.number().allow(null),
  extraOptions: Joi.array().items(extraOptionSchema).min(0),
  pickupPreferenceId: Joi.number().integer().allow(null),
  additionalStopId: Joi.number().integer().allow(null),
  additionalStopOnTheWayDescription: Joi.string().allow(""),
  ...
  const addr =
    typeof value.airportLocationAddress === "string"
      ? value.airportLocationAddress.trim()
      : "";
  const lat = value.airportLocationLatitude;
  const lng = value.airportLocationLongitude;
  const hasGeoTerminal =
    addr.length > 0 &&
    lat !== undefined &&
    lat !== null &&
    lng !== undefined &&
  ...
  if (!hasCatalogAirport && !hasGeoTerminal) {
    return helpers.message(
      "Provide either airportId (catalog airport) or airport terminal: airportLocationAddress, airportLocationLatitude, airportLocationLongitude"
    );
  }
  if (hasCatalogAirport && hasGeoTerminal) {
    return helpers.message(
      "Send either airportId or airport location fields (address + coordinates), not both"
    );
  }
  return value;
});
 ...
 * Note: airport location must still be provided via either airportId or airportLocation* when updating them.
 */
const validateAirportBookUpdate = Joi.object({
  tripType: Joi.string().valid(
    "Ride to the airport(one way)",
    "Ride from the airport(one way)",
    "Ride to the airport(round trip)",
    "Ride from the airport(round trip)"
  ...
  accommodationAddress: Joi.string(),
  accommodationLongitude: Joi.number(),
  accommodationLatitude: Joi.number(),
  pickupDateTime: Joi.string()
    .regex(dateFormat)
    .message("Invalid pickup Date format. " + dateFormatMessage),
  returnPickupDateTime: Joi.string()
  ...
  const touchesAirport =
    Object.prototype.hasOwnProperty.call(value, "airportId") ||
    Object.prototype.hasOwnProperty.call(value, "airportLocationAddress") ||
    Object.prototype.hasOwnProperty.call(value, "airportLocationLatitude") ||
    Object.prototype.hasOwnProperty.call(value, "airportLocationLongitude");
  if (!touchesAirport) return value;
  ...
  if (!hasCatalogAirport && !hasGeoTerminal) {
    return helpers.message(
      "To update airport location, provide either airportId or airportLocationAddress + airportLocationLatitude + airportLocationLongitude"
    );
  }
  if (hasCatalogAirport && hasGeoTerminal) {
    return helpers.message(
      "To update airport location, send either airportId or airport location fields (address + coordinates), not both"
    );
  }
  return value;
});
